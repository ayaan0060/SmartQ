...
const CONFIG = {
  API_BASE: window.location.hostname === 'localhost' && window.location.port === '8000'
    ? 'http://localhost:8000'   // served directly from triage backend
    : 'http://localhost:8000',  // always point to triage backend
  GOOGLE_MAPS_API_KEY: '',
...
function initChat() {
  ...
  // Welcome message using actual user name
  const userName = document.getElementById('sidebar-user-name')?.textContent || 'there';
  appendBotMessage(
    `👋 Hello ${userName}! Welcome to **MedTriage AI**.\n\n` +
...
async function handleAPIResponse(data) {
  ...
  // Handle triage result — check both data.triage and data.urgency_summary
  const triage = data.triage || (data.urgency_summary ? {
    urgency_level: data.urgency_summary.urgency_level,
    recommended_action: data.urgency_summary.action,
    possible_conditions: data.urgency_summary.conditions,
    urgency_score: 70,
    confidence: 0.9,
  } : null);

  if (triage) {
    updateTriageLevel(triage.urgency_level, triage.recommended_action);
    state.conditions = triage.possible_conditions || [];
    updateConditionsDisplay();
    state.reportData = buildReportData({ ...data, triage });
  }

  const replyText = data.reply || data.urgency_summary?.message || 'Thank you for the information.';
  const isEmergency = triage && ['CRITICAL', 'HIGH'].includes(triage.urgency_level);

  appendBotMessage(replyText, isEmergency, triage);
}
...
function appendUserMessage(text) {
  ...
  const initials = document.getElementById('user-initials')?.textContent || 'P';
  bubble.innerHTML = `
    <div class="user-avatar-chat">${initials}</div>
...
// ══════════════════════════════════════════════════════════
// 7. MAPLIBRE GL JS INTEGRATION (REPLACING GOOGLE MAPS)
// ══════════════════════════════════════════════════════════
function loadGoogleMap() {
  // We keep the function name loadGoogleMap for compatibility with the rest of the script calls
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(pos => {
      ...
      renderMapLibre(container, lat, lng);
    }, () => {
      renderMapLibre(container, 28.6139, 77.2090); // Default New Delhi
    });
  } else {
    renderMapLibre(container, 28.6139, 77.2090);
  }
}

function renderMapLibre(container, lat, lng) {
  if (!window.maplibregl) {
    container.innerHTML = '<div style="padding:20px;text-align:center;color:white">MapLibre library not loaded.</div>';
    return;
  }
  ...
  state.googleMap = map; // Reusing state key for simplicity
  ...
  // Add dummy hospital markers for demo
  const hospitals = [
    { name: "City General Hospital", lat: lat + 0.005, lng: lng + 0.008 },
    { name: "MedPlus Urgent Care", lat: lat - 0.007, lng: lng - 0.004 }
  ];

  hospitals.forEach(h => {
    new maplibregl.Marker({ color: '#ef4444' })
      .setLngLat([h.lng, h.lat])
      .setPopup(new maplibregl.Popup().setHTML(`<b>${h.name}</b>`))
      .addTo(map);
  });

  const sub = $('map-sub-text');
  if (sub) sub.textContent = `Showing medical facilities near your location`;
  $('facilities-count').textContent = '2 nearby';
}
...
function resetSession() {
  ...
  if (msgs) {
    ...
    msgs.innerHTML = '';
    appendBotMessage(
      `✨ **New session started.** Hello ${userName}! Please describe your current symptoms and I'll assess your care needs.\n\n` +
...
document.addEventListener('DOMContentLoaded', () => {
  // Load user from URL params (passed by SmartQ) or fallback to localStorage
  try {
    ...
    if (!name) {
      const raw = localStorage.getItem('smartq-auth-storage') || '{}';
    ...
    if (name) {
      ...
      document.getElementById('sidebar-user-role').textContent = `${role || 'Patient'} · SmartQ`;
      CONFIG.DEMO_PATIENT.name = name;
    } else {
      document.getElementById('user-initials').textContent   = 'P';
      document.getElementById('sidebar-user-name').textContent = 'Guest Patient';
      document.getElementById('sidebar-user-role').textContent = 'Patient · SmartQ';
    }
  } catch {
    document.getElementById('user-initials').textContent   = 'P';
    document.getElementById('sidebar-user-name').textContent = 'Guest Patient';
  ...
  fetch(`${CONFIG.API_BASE}/health`, { signal: AbortSignal.timeout(5000) })
    .then(r => r.json())
