...
// ── Hospital Map (MapLibre + OSM) ──────────────────────────────────────────
function HospitalMap({ lat, lng, hospitals }) {
  ...
  useEffect(() => {
    ...
    // Use CartoDB Dark Matter (Great for dark theme!)
    ...
    // Add User Marker (Patient)
    new maplibregl.Marker({ color: T.purple })
      .setLngLat([lng, lat])
      ...
      new maplibregl.Marker({ element: el })
        ...
        .setPopup(new maplibregl.Popup({ offset: 25 }).setHTML(`
          <div style="color: #333; padding: 5px;">
            ...
            <span style="font-size: 12px; color: #666;">${h.distance_km} km away</span><br/>
            <div style="margin-top: 8px;">
              <a href="${h.directions_url}" target="_blank" style="
                background: ${T.purple}; color: #fff; text-decoration: none;
  ...
  return (
    <div style={{ position: "relative", marginTop: 16 }}>
      <div style={S.mapHeader}>📍 Nearby Hospitals (Live Map)</div>
...
function BotResponse({ msg, sessionId }) {
  ...
  // Fetch nearby hospitals for high urgency
  useEffect(() => {
    if (triage && (triage.urgency_level === "CRITICAL" || triage.urgency_level === "HIGH")) {
      setMapLoading(true);
      navigator.geolocation.getCurrentPosition(
        async (pos) => {
          ...
          setLoc({ lat, lng });
          try {
            const res = await axios.get(`${API}/hospitals/nearby`, {
              params: { lat, lng, urgency: triage.urgency_level }
            });
        ...
        () => {
          // Fallback (e.g. Bhubaneswar if denied)
          const lat = 20.2961, lng = 85.8245;
          setLoc({ lat, lng });
          setMapLoading(false);
        }
      );
    ...
    <div className="msgSlide" style={{ marginBottom: 20 }}>
      ...
      {triage && cfg && (
        <div className="triageReveal" style={{
          ...
          {/* Interactive Map (New) */}
          {(triage.urgency_level === "CRITICAL" || triage.urgency_level === "HIGH") && (
            <div style={{ marginTop: 16 }}>
              {mapLoading && <div style={{ fontSize: 12, color: T.textMuted, textAlign: "center", padding: 20 }}>⏳ Initializing live map...</div>}
              {!mapLoading && loc && <HospitalMap lat={loc.lat} lng={loc.lng} hospitals={hospitals} />}
            </div>
          )}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <button
          onClick={() => { window.open(`https://www.openstreetmap.org/search?query=hospital+near+me#map=14/${triage?.location?.lat || 20.296}/${triage?.location?.lng || 85.824}`, "_blank"); }}
          style={{
